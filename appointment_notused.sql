...
ISNULL(cast(ta.Date as TIME) ,'') AS startTime,
ISNULL(State.Name ,'') AS leadState,
ISNULL(ta.UserId,'') AS assignee,
ISNULL( CASE WHEN ConfiguredStatus.IsContractSigned = 0 THEN ta.ContractId END ,'') AS lead,
ISNULL(CASE WHEN ConfiguredStatus.IsContractSigned = 1 THEN ta.ContractId END ,'') AS client,
...
AND client.ContractApplicant.IsPrimary=1